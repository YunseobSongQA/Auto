...
이 저장소는 **QA 설계부터 자동화 실행까지 5가지 도구**를 모은 QA 포트폴리오입니다.
앞단의 **PRD2TC**(기획서 → 테스트케이스 자동 생성)는 별도 배포 웹앱이라 이 저장소에 코드 폴더가 없고,
이 코드 가이드는 **하나의 대상([QASS](https://qass1.pages.dev/))을 자동화하는 4개 도구**를 다룹니다.
...
```
 ...
 JS/TS           Python            Newman(JS)        Python
 video 녹화       Xvfb+ffmpeg 녹화   부하·성능 측정     pytest 러너
   │               │                │                │
   └───────────────┴────────┬───────┴────────────────┘
                            ▼
...
| **Playwright (레퍼런스)** | [`playwright/qass-flow.js`](./playwright/qass-flow.js) | 핵심 플로우 구현(JS/TS) — 다른 도구가 따라가는 원본 |
| **Selenium** | [`selenium/qass_flow.py`](./selenium/qass_flow.py) | 같은 8스텝을 WebDriver로 (Python) |
...
| 항목 | Playwright (JS/TS) | Selenium (Python) | Appium (Python) | API (JS) |
|------|-----------|----------|--------|-----|
...
# Selenium (헤드리스, 또는 ./record.sh 로 녹화 · Python)
cd selenium && pip install -r requirements.txt && python run.py
# 쇼케이스 웹
cd web && python3 -m http.server 8080
# Appium (맨 뒤 · 안드로이드 기기 필요 · PC에서 · Python)
cd appium && pip install -r requirements.txt && appium driver install uiautomator2 && pytest -s
```
